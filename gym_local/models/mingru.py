...
class MinGRU(nn.Module):
    """
    Minimal Parallel GRU implementation using log-space computations
    ...
    def __init__(self, input_dim: int, expansion_factor: int = 1):
        ...
        self.projector = nn.Linear(self.input_dim, self.hidden_dim*2, bias=False)
    ...
    def forward(self, inputs: torch.Tensor, prev_hidden: Union[None, torch.Tensor] = None) -> Tuple[torch.Tensor, Union[None, torch.Tensor]]:
        """
        Forward pass of the minGRU model
        ...
        log_forget, log_update, log_hidden = self._compute_gate_parameters(inputs)

        log_candidate = log_update + log_hidden
    ...
    def _compute_gate_parameters(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        hidden, gates = self.projector(inputs).chunk(2, dim=-1)

        # Compute log space parameters
        log_update = -F.softplus(-gates)
        log_forget = -F.softplus(gates)
        log_hidden = self._activation(hidden, in_log_space=True)

        return log_forget, log_update, log_hidden
